								   ...
								   enum streamtype type)
{
	...
	{ // début de stream
		...
		pthread_mutex_lock(&mutex_for_hashmap);
		if (type == TYPE_THEORA)
			HASH_ADD_INT(theorastrstate, serial, s);
		else
			HASH_ADD_INT(vorbisstrstate, serial, s);
		pthread_mutex_unlock(&mutex_for_hashmap);
	}
	else
	{
		// proteger l'accès à la hashmap

		//Q3 : La structure, coté vidéo, est maniée par deux threads. Il faudra donc la protéger des accès concurrents.

		//int pthread_mutex_lock(pthread_mutex_t *mutex); : permet de verrouiller le mutex
		...
		//pthread_mutex_unlock() vérifie lors de l'exécution que le mutex est verrouillé en entrant,
		//et qu'il est verrouillé par le même thread que celui appelant pthread_mutex_unlock()
		pthread_mutex_unlock(&mutex_for_hashmap);
		...
		s->strtype != TYPE_VORBIS)
	{
		...
		if (res != TH_ENOTFORMAT)
		{
			...
			if (type == TYPE_THEORA)
			{
				...
				/*Q2 : Vous devez aussi lancer le thread gérant l’affichage en exécutant la
		fonction draw2SDL. Le lancement a lieu vers la ligne 144 du fichier
		stream_common.c dans la fonction decodeAllHeaders. Il prend en
		argument le numéro du flux vidéo (s->serial).*/

				// inserer votre code ici !!
				/*thread already defined in stream_common.h extern pthread_t theora2sdlthread;*/
				//pthread_t theora2sdlthread;

				res = pthread_create(&theora2sdlthread, NULL, draw2SDL, (void *)(long long int)s->serial);
